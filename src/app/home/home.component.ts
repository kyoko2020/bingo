...
export class HomeComponent implements OnInit {
  ...
  ngOnInit():void {
    ...
    //1-75を配列に追加
  this.targetNumber = (function(from, to) {
      for(var tmp = []; from <= to; from++) {
          tmp.push(('00' + from).slice(-2));
          }
          return tmp;
      })(1, 75);
  ...
  staBtn():void{
    ...
    if(this.btnName === "START"){
      this.timerId = setInterval( () => {
        var onetargetIndex = Math.floor(Math.random() * this.targetNumber.length).toString(); 
      ...
      if(this.delArrayNum >= 0){
        ...
        // if(this.winningNumber.length > 1){
        // //表示用に配列に保持
        // this.winningNumber.shift();
        // }
        //targetNumber配列から削除
